"""
Standalone test for validators (to avoid full package initialization).
Run with: python test_validators_standalone.py
"""
...
from unittest.mock import Mock  # noqa: E402 - import after intentional pre-import setup

from circuitkit.corruption.validators import (  # noqa: E402 - import after intentional pre-import setup
    CompositeValidator,
...
def test_length_budget_validator():
    ...
    print("  ✓ Valid corruption (exact length)")
    ...
    print("  ✓ Invalid corruption (too long)")
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is False
    print("  ✓ Invalid corruption (missing prompt)")
...
def test_label_consistency_validator():
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is True
    print("  ✓ Valid corruption (answer present)")
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is False
    print("  ✓ Invalid corruption (answer missing)")
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is True
    print("  ✓ Valid corruption (case insensitive)")
...
def test_tokenization_validator():
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is True
    print("  ✓ Valid corruption (exact token length)")
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is True
    print("  ✓ Valid corruption (within tolerance)")
    ...
    result = validator.validate(clean, corrupted)
    assert result.is_valid is False
    print("  ✓ Invalid corruption (exceeds tolerance)")
...
def test_validation_result():
    ...
    print("  ✓ Dataclass creation and access")
