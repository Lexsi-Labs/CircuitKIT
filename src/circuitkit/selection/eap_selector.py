"""EAP / EAP-IG selectors — use circuitkit's Graph + attribute_node."""
import torch
import logging
...
def _run_eap_on_model(model, task_name: str, algorithm: str, config: dict) -> dict:
    """Run EAP or EAP-IG attribution on an already-loaded HookedTransformer."""
...
@register("eap")
def eap_selector(model, task_name: str, config: dict) -> dict:
    return _run_eap_on_model(model, task_name, "eap", config)


@register("eap-ig")
def eap_ig_selector(model, task_name: str, config: dict) -> dict:
    return _run_eap_on_model(model, task_name, "eap-ig", config)
