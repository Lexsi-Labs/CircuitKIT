...
from ..backends.eap.eap_utils import (
...
def evaluate_graph(  # noqa: C901 - complex function, refactor out of scope for lint pass
    ...
    Args:
        ...
        skip_clean (bool): If True, clean_logits passed to metrics will be None.
            ...
            Defaults to True.
        exclude_misaligned (bool): If True, drop pairs whose clean/corrupted token
            counts differ from the returned per-sample scores. EAP integrates
            per-position (corrupted - clean) activation differences, and every
            metric indexes the answer at ``logits[i, input_length - 1]`` — a
            single contiguous real-token region. A pair with unequal token counts
            (e.g. a mid-sequence distractor insert or multi-token entity swap)
            cannot be corrected by end-padding alone: mid-sequence realignment
            would insert padding before the answer position and break that
            contiguous-region assumption for every metric, which would be worse
            than the status quo. Excluding those pairs from the score is the
            sound alternative; the count excluded is logged once (via the same
            throttled warning ``tokenize_batch_pair`` already emits). Defaults to
            False (score every pair, matching prior behavior).
            Requires a per-sample metric (``mean=False, loss=False`` — see
            ``circuitkit.api._make_eval_metric``); the builtin tasks'
            ``task_spec.metric_fn()`` returns ``mean=True`` by default, which
            aggregates to one score per batch rather than one per sample. Passing
            such a metric with exclude_misaligned=True raises ValueError rather
            than silently mis-filtering.

    Returns:
        Union[Tensor, List[Tensor]]: Per-sample metric scores [n_samples] (or fewer,
            if exclude_misaligned dropped any pairs), or a list of such tensors if
            multiple metrics were provided.
    ...
    # `templated` (set by build_dataloader from the task's chat_template_mode)
    # decides whether the clean/corrupted strings already carry a chat-template
    # BOS; it must be forwarded so tokenization matches circuit discovery and a
    # second BOS is not injected. Defaults False -> legacy raw-text behavior.
    ...
    if "mean" in intervention:
        ...
        means = compute_mean_activations(
            ...
            templated=getattr(intervention_dataloader, "templated", False),
        )
    ...
    for batch_data in dataloader:
        # Handle both old format (clean, corrupted, label) and new format
        # (clean, corrupted, label, answer_spans) from collate_EAP_with_spans
        if len(batch_data) == 4:
            clean, corrupted, label, answer_spans = batch_data
        else:
        ...
        if exclude_misaligned:
            n_real_clean = clean_attention_mask.sum(dim=1)
            n_real_corr = corrupted_attention_mask.sum(dim=1)
            keep_mask = n_real_clean == n_real_corr
            n_excluded = int((~keep_mask).sum().item())
            if n_excluded:
                logger.warning(
                    "evaluate_graph: excluding %d of %d pair(s) with unequal clean/corrupt "
                    "token counts from the score (exclude_misaligned=True).",
                    n_excluded,
                    len(clean),
                )
        ...
        for i, metric in enumerate(metrics):
            # Check if metric accepts answer_spans parameter
            sig = inspect.signature(metric)
            if "answer_spans" in sig.parameters:
                r = metric(
                    logits, clean_logits, input_lengths, label, answer_spans=answer_spans
                ).cpu()
            else:
            ...
            if keep_mask is not None:
                if r.shape[0] != keep_mask.shape[0]:
                    raise ValueError(
                        f"exclude_misaligned=True requires a per-sample metric (one score "
                        f"per batch row), but metric #{i} returned {r.shape[0]} score(s) for "
                        f"a batch of {keep_mask.shape[0]}. This usually means the metric was "
                        f"built with mean=True/loss=True (an aggregated scalar) instead of "
                        f"per-sample scores — e.g. task_spec.metric_fn() returns "
                        f"functools.partial(..., mean=True) by default for the builtin tasks. "
                        f"Rebuild it with mean=False, loss=False (see "
                        f"circuitkit.api._make_eval_metric), or don't pass "
                        f"exclude_misaligned=True with an aggregating metric."
                    )
                r = r[keep_mask.cpu()]
            results[i].append(r)
...
def evaluate_baseline(
    ...
    Args:
        ...
        quiet (bool): Suppress tqdm progress bar. Defaults to False.
        exclude_misaligned (bool): If True, drop pairs whose clean/corrupted token
            counts differ, matching evaluate_graph's exclude_misaligned. Pass the
            same value here as to the paired evaluate_graph call (e.g. within a
            pillar's clean/corrupt/circuit passes) so all three scores are computed
            over the identical example set. Defaults to False.
    ...
    results = [[] for _ in metrics]
    pair_padding_side = getattr(dataloader, "pair_padding_side", None)
    # Forwarded so baseline tokenization matches chat-templated discovery
    # (see note in evaluate_graph). Defaults False -> legacy raw-text behavior.
    templated = getattr(dataloader, "templated", False)
    if not quiet:
        dataloader = tqdm(dataloader)
    for batch_data in dataloader:
        # Handle both old format (clean, corrupted, label) and new format
        # (clean, corrupted, label, answer_spans)
        if len(batch_data) == 4:
            clean, corrupted, label, answer_spans = batch_data
        else:
            clean, corrupted, label = batch_data
        ...
        if exclude_misaligned:
            ...
            if n_excluded:
                logger.warning(
                    "evaluate_baseline: excluding %d of %d pair(s) with unequal clean/corrupt "
                    "token counts from the score (exclude_misaligned=True).",
                    n_excluded,
                    len(clean),
                )
        ...
        for i, metric in enumerate(metrics):
            ...
            if "answer_spans" in sig.parameters:
                if run_corrupted:
                    r = metric(
                        corrupted_logits, logits, input_lengths, label, answer_spans=answer_spans
                    ).cpu()
                else:
                    r = metric(
                        logits, corrupted_logits, input_lengths, label, answer_spans=answer_spans
                    ).cpu()
            else:
                if run_corrupted:
...
def evaluate_ibcircuit_neuron_circuit(  # noqa: C901 - complex function, refactor out of scope for lint pass
    ...
    pair_padding_side = getattr(dataloader, "pair_padding_side", None)
    # Forwarded so IBCircuit tokenization matches chat-templated discovery
    # (see note in evaluate_graph). Defaults False -> legacy raw-text behavior.
    templated = getattr(dataloader, "templated", False)
            ...
            def hook_fn(z, hook):  # z: [batch, pos, n_heads, d_head]
                ...
                for head_idx, neuron_idx in hd.items():
                    if intervention == "zero":
                        z[:, :actual_pos, head_idx, neuron_idx] = 0
                    else:
    ...
    for batch_data in dl:
        ...
        for i, metric in enumerate(metrics):
            sig = inspect.signature(metric)
            if "answer_spans" in sig.parameters:
                r = metric(logits, None, input_lengths, label, answer_spans=answer_spans).cpu()
            else:
