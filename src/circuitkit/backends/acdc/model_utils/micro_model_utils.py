...
class Block(t.nn.Module):
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:  # shape: (batch, seq, resid)
...
def factorized_src_nodes(model: MicroModel) -> Set[SrcNode]:
    """Get the source nodes for the factorized graph."""
    nodes = set()
...
def factorized_dest_nodes(model: MicroModel) -> Set[DestNode]:
    """Get the destination nodes for the factorized graph."""
    nodes = set()
    layers = count(1)
    for layer_idx in range(model.n_layers):
        layer = next(layers)
