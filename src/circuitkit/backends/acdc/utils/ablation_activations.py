...
from ..data import BatchKey, PromptDataLoader
from ..types import AblationType, SrcNode
from ..utils.misc import remove_hooks
...
def src_out_hook(
    ...
    elif ablation_type == AblationType.TOKENWISE_MEAN_CLEAN:
        # Use clean dataset mean for tokenwise ablation
        out = out.mean(dim=0, keepdim=True).repeat([out.size(0)] + [1] * (out.ndim - 1))
    elif ablation_type == AblationType.TOKENWISE_MEAN_CORRUPT:
        # Use corrupt dataset mean for tokenwise ablation
        out = out.mean(dim=0, keepdim=True).repeat([out.size(0)] + [1] * (out.ndim - 1))
    elif ablation_type == AblationType.TOKENWISE_MEAN_CLEAN_AND_CORRUPT:
        # Use combined clean and corrupt dataset mean
        out = out.mean(dim=0, keepdim=True).repeat([out.size(0)] + [1] * (out.ndim - 1))
    else:
        raise NotImplementedError(f"Unsupported ablation type: {ablation_type}")
...
def src_ablations(
    ...
    """
    Get the activations used to ablate each `Edge` in a model, given a particular
    set of model inputs and an ablation type. See `AblationType` for details.

    Args:
        model: The model to get the ablations for.
        sample: The data sample to get the ablations for. Can be a single batch of
            inputs or a DataLoader.
        ablation_type: The type of ablation to perform.

    Returns:
        A tensor of activations used for ablation.
        Shape is `[Srcs, ...]` where `Srcs` is the number of `SrcNode`s in the model
        and `...` is the shape of the activations (e.g., `[batch, seq, d_model]`).
    """
    src_outs: Dict[SrcNode, t.Tensor] = {}
