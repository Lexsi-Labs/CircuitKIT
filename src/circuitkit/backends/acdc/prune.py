...
from .data import PromptDataLoader  # Corrected import path
from .types import (  # Corrected import path
    AblationType,
    CircuitOutputs,
    PatchType,
    PatchWrapper,
...
from .utils.ablation_activations import src_ablations  # Corrected import path
from .utils.custom_tqdm import tqdm  # Corrected import path
from .utils.graph_utils import patch_mode  # Corrected import path
from .utils.misc import module_by_name  # Corrected import path
from .utils.patchable_model import PatchableModel  # Corrected import path
from .utils.tensor_ops import desc_prune_scores, prune_scores_threshold  # Corrected import path
from .visualize import draw_seq_graph  # Corrected import path
...
def run_circuits(
    ...
    Args:
        ...
        test_edge_counts: The numbers of edges to keep in the circuit.
    ...
    Returns:
        A dictionary mapping from the number of pruned edges to the model's
        batched outputs.
    """
    ...
    # <--- START OF CHANGE --->
    # Dynamically get the model's dtype to support float32, float16, bfloat16
    ...
    for batch_idx, batch in enumerate(batch_pbar := tqdm(dataloader)):
        ...
        if (patch_type == PatchType.TREE_PATCH and not reverse_clean_corrupt) or (
            ...
            # Default case - use clean input and corrupt ablations
            batch_input = batch.clean
            if not ablation_type.mean_over_dataset:
                patch_src_outs = src_ablations(model, batch.corrupt, ablation_type)
            ...
            for edge_count in (edge_pbar := tqdm(test_edge_counts)):
                ...
                for mod_name, patch_mask in prune_scores.items():
                    ...
                    if patch_type == PatchType.EDGE_PATCH:
                        # <--- START OF CHANGE --->
                        # Replaced .float() with .to(model_dtype)
                        dest.patch_mask.data = (patch_mask.abs() >= threshold).to(model_dtype)
                        # <--- END OF CHANGE --->
