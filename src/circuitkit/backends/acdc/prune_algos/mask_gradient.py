...
from ..types import AblationType, BatchKey, Edge, PruneScores
from ..utils.ablation_activations import batch_src_ablations
from ..utils.custom_tqdm import tqdm
from ..utils.graph_utils import patch_mode, set_all_masks, train_mask_mode
from ..utils.patchable_model import PatchableModel
from ..utils.tensor_ops import batch_avg_answer_diff, batch_avg_answer_val
...
def mask_gradient_prune_scores(
    ...
    """
    Calculate prune scores based on the gradient of the mask values. These masks
    interpolate edges between clean and ablated activations.

    This method is equivalent to Edge Attribution Patching (EAP) when
    `grad_function="logit"`, `answer_function="avg_diff"`, and `mask_val=0.0`.
    ...
    Args:
        ...
        integrated_grad_samples: If not `None`, approximates Integrated Gradients by
            averaging gradients over `integrated_grad_samples` mask values
        ...
        clean_corrupt: Whether to use the clean or corrupt inputs to calculate the
            ablations.
        accumulation_steps: The number of batches to accumulate gradients over before
            updating the scores. Reduces peak memory usage.

    Returns:
        An ordering of the edges by importance. Importance is equal to the
        absolute value of the score assigned to the edge.
    """
    assert (mask_val is not None) ^ (integrated_grad_samples is not None)  # XOR
    ...
    prune_scores: PruneScores = model.new_prune_scores(init_val=0.0)
    ...
    with train_mask_mode(model):
        for sample in (ig_pbar := tqdm(range((integrated_grad_samples or 0) + 1))):
            ...
            # Zero out gradients before starting a new sample run
            for dest_wrapper in model.dest_wrappers:
                if dest_wrapper.patch_mask.grad is not None:
                    dest_wrapper.patch_mask.grad.zero_()
            ...
            for i, batch in enumerate(dataloader):
                ...
                del loss, logits, token_vals, patch_src_outs
                t.cuda.empty_cache()
                gc.collect()
                ...
                if (i + 1) % accumulation_steps == 0 or (i + 1) == len(dataloader):
                    ...
                    for dest_wrapper in model.dest_wrappers:
                        ...
                        # Zero gradients after accumulating
                        if dest_wrapper.patch_mask.grad is not None:
                            dest_wrapper.patch_mask.grad.zero_()
