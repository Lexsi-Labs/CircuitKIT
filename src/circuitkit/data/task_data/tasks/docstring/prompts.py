...
class Prompt:
    ...
    def _print_prompt(self, prompt):
        logger.info(f"{prompt}|{self.correct_answers[0]}| <===")
    ...
    def _print_tokenized(self, model, prompt):
        tokens = "|" + "|".join(model.to_str_tokens(prompt)) + "|"
...
class BatchedPrompts:
    ...
    def correct_prob(
        self,
        logits: TT["batch", "pos", "d_vocab"],  # noqa: F821 - jaxtyping shape annotation
        pos: int = -1,
    ) -> TT["batch"]:  # noqa: F821 - jaxtyping shape annotation
