...
from ...core.acdc_utils import kl_divergence
from ...core.TLACDCEdge import TorchIndex

# from acdc.ioi.ioi_dataset import IOIDataset  # Not needed for greaterthan
...
class AllDataThings:
    ...
    # The greater-than generator builds its own model and stores it here.
    tl_model: Any = None


from ...tasks.ioi.utils import (  # noqa: E402 - import after intentional pre-import setup
    get_gpt2_small,
)
...
class GreaterThanConstants:
    ...
    @classmethod
    def get(cls: type["GreaterThanConstants"], model) -> "GreaterThanConstants":
        if cls._instance is None:
            # The metric functions only have a device/logits handle, not the
            # model, so they rely on the singleton already being built by
            # get_year_data(). If a bare device string slips through, fail
            # with an actionable message instead of trying to load a model
            # literally named "cpu"/"cuda".
            if not hasattr(model, "tokenizer"):
                raise RuntimeError(
                    "GreaterThanConstants has not been initialized yet and "
                    f"get() was called with {model!r} (not a model). Call "
                    "get_year_data(num_examples, model) first to build it."
                )
    ...
    def __init__(self, model):
        """
        Initialize GreaterThanConstants with a model.

        Args:
            model: HookedTransformer model instance (REQUIRED, no default model)

        Raises:
            ValueError: If model is None
        """
        if model is None:
            raise ValueError("Model required for GreaterThanConstants. No default model.")

        from .....utils.token_utils import TokenIDGenerator

        self.token_gen = TokenIDGenerator(model)
        ...
        vocab_size = self.token_gen.vocab_size
        INV_TOKENS_TENSOR = torch.zeros(vocab_size, dtype=torch.long)
        for i, v in enumerate(TOKENS_TENSOR):
            if v < vocab_size:  # Safety check
...
def get_year_data(num_examples, model):
    # GreaterThanConstants.get() builds a singleton from the *model* (it needs
    # model.tokenizer). Passing model.cfg.device here would feed a device
    # string ("cpu") into the constructor and TokenIDGenerator would try to
    # load a model named "cpu". Pass the model itself.
...
def get_all_greaterthan_things(num_examples, metric_name, device=None):
...
def get_greaterthan_true_edges(  # noqa: C901 - complex function, refactor out of scope for lint pass
    model,
):
    ...
    def tuple_to_hooks(layer_idx, head_idx, outp=False):
        if outp:
            if head_idx is None:
                ...
                return [
                    (f"blocks.{layer_idx}.attn.hook_result", TorchIndex([None, None, head_idx]))
                ]
