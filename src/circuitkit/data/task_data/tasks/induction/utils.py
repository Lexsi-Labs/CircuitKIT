...
from ...core.acdc_utils import MatchNLLMetric, kl_divergence, negative_log_probs, shuffle_tensor
...
class AllDataThings:
    ...
    # The induction generator builds its own model internally and stores it
    # here so callers can reuse it (the other tasks do not populate this).
    tl_model: Any = None
...
def get_validation_data(num_examples=None, seq_len=None, device=None):
    ...
    validation_data = torch.load(validation_fname, map_location=device, weights_only=True).long()
...
def get_good_induction_candidates(num_examples=None, seq_len=None, device=None):
    ...
    good_induction_candidates = torch.load(good_induction_candidates_fname, map_location=device, weights_only=True)
...
def one_item_per_batch(
    toks_int_values, toks_int_values_other, mask_rep, base_model_logprobs, kl_take_mean=True
):
