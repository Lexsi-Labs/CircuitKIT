...
from .acdc_utils import extract_info, next_key, shuffle_tensor
from .TLACDCCorrespondence import TLACDCCorrespondence
from .TLACDCEdge import EdgeType, TorchIndex  # these introduce several important classes !!!
from .TLACDCInterpNode import TLACDCInterpNode
...
class TLACDCExperiment:
    ...
    def __init__(
        ...
        if zero_ablation and remove_redundant:
            raise ValueError(
                "It's not possible to do zero ablation and remove redundant paths.\
        ...
        self.global_cache = GlobalCache(  # noqa: F821 - pre-existing vendored bug
            device=(
                "cpu" if self.online_cache_cpu else "cuda",
                "cpu" if self.corrupted_cache_cpu else "cuda",
            ),
        )
        ...
        assert (
            self.ref_ds is not None or self.zero_ablation
        ), "If you're doing random ablation, you need a ref ds"
    ...
    def sender_hook(self, z, hook, verbose=False, cache="online", device=None):
        ...
        if verbose:
            pass  # Log only if verbose mode is enabled in the experiment
    ...
    def receiver_hook(self, hook_point_input, hook, verbose=False):
        ...
        if verbose:
            pass  # Log receiver hook entry if needed
        ...
        if EdgeType.DIRECT_COMPUTATION in incoming_edge_types:
            ...
            )  # It is crucial to use [:] to not use same tensor

            if verbose:
                pass  # Log cache overwrite
            ...
            for receiver_index in self.corr.edges[hook.name]:
                ...
                ), "This is a direct computation, so there should only be one sender node"
                if len(list_of_senders) == 0:
                    continue
                sender_node = list_of_senders[0]

                sender_indices = list(
                    self.corr.edges[hook.name][receiver_index][sender_node].keys()
                )
                assert (
        ...
        for receiver_node_index in self.corr.edges[hook.name]:
            for sender_node_name in self.corr.edges[hook.name][receiver_node_index]:
                for sender_node_index in self.corr.edges[hook.name][receiver_node_index][
                    sender_node_name
                ]:
                    ...
                    if verbose:
                        pass  # Log edge details if needed

                    if edge.edge_type == EdgeType.ADDITION:
                        # Add the effect of the new head (from the current forward pass)
                        hook_point_input[
                            receiver_node_index.as_index
                        ] += self.global_cache.online_cache[sender_node_name][
                            sender_node_index.as_index
                        ].to(
                            hook_point_input.device
                        )
                        ...
                        hook_point_input[
                            receiver_node_index.as_index
                        ] -= self.global_cache.corrupted_cache[sender_node_name][
                            sender_node_index.as_index
                        ].to(
                            hook_point_input.device
                        )
        ...
        for big_tuple, edge in self.corr.all_edges().items():
            ...
            for node in nodes:
                ...
                if len(fwd_hooks) > 0 and not sender_and_receiver_both_ok:
                    resolved_hooks_dicts = [
                        fwd_hook.hook.hooks_dict_ref() for fwd_hook in fwd_hooks
                    ]
                    assert all(
                        [
                            resolved_hooks_dict == resolved_hooks_dicts[0]
                            for resolved_hooks_dict in resolved_hooks_dicts
                        ]
                    ...
                    for fwd_hook in resolved_hooks_dicts[0].values():
                        if isinstance(fwd_hook, partial):
                            hook_func_name = fwd_hook.__wrapped__.__name__
                        else:
                            hook_func_name = fwd_hook.__name__
                        assert (
                ...
                self.model.add_hook(
                    name=node.name,
                    hook=partial(
                        self.sender_hook, verbose=self.hook_verbose, cache=cache, device=device
                    ),
                )
            ...
            receiver_node_names = list(
                set(
                    [
                        ...
                        if node.incoming_edge_type != EdgeType.PLACEHOLDER
                    ]
                )
            )
            for receiver_name in receiver_node_names:
                self.model.add_hook(
                    name=receiver_name,
    ...
    def save_edges(self, fname):
        """Save edges as a picklable list for fast I/O."""
        edges_list = []
    ...
    def add_sender_hook(self, node, override=False):
        if not override and len(self.model.hook_dict[node.name].fwd_hooks) > 0:
            ...
            if len(fwd_hooks) > 0:
                ...
                for fwd_hook in resolved_hooks_dicts[0].values():
                    hook_func_name = (
                        fwd_hook.__wrapped__.__name__
                        if isinstance(fwd_hook, partial)
                        else fwd_hook.__name__
                    )
                    assert (
                        "sender_hook" in hook_func_name
                    ), f"You should only add sender hooks to {node.name}, and this: {hook_func_name} doesn't look like a sender hook"
    ...
    def add_receiver_hook(self, node, override=False, prepend=False):
        if (
            not override and len(self.model.hook_dict[node.name].fwd_hooks) > 0
        ):  # repeating code from add_sender_hooks
            fwd_hooks = self.model.hook_dict[node.name].fwd_hooks
            if len(fwd_hooks) > 0:
                resolved_hooks_dicts = [fwd_hook.hook.hooks_dict_ref() for fwd_hook in fwd_hooks]
                assert all(
    ...
    def step(self, early_stop=False, testing=False):
        ...
        for sender_name in sender_names_list:
            sender_indices_list = list(
                self.corr.edges[self.current_node.name][self.current_node.index][sender_name]
            )
            ...
            for sender_index in sender_indices_list:
                edge = self.corr.edges[self.current_node.name][self.current_node.index][
                    sender_name
                ][sender_index]
                ...
                self.update_cur_metric(
                    recalc_edges=False
                )  # warning: gives fast evaluation, though edge count is wrong
                evaluated_metric = (
                    self.cur_metric
                ...
                if (
                    early_stop
                ):  # for debugging the effects of one and only one forward pass WITH a corrupted edge
                    return
                ...
                self.update_cur_metric(
                    recalc_edges=False, recalc_metric=False
                )  # So we log current state to wandb

                if self.using_wandb:
                    log_metrics_to_wandb(  # noqa: F821 - pre-existing vendored bug
                        self,
        ...
        if (
            is_this_node_used
            and self.current_node.incoming_edge_type.value != EdgeType.PLACEHOLDER.value
        ):
            fname = f"ims/img_new_{self.step_idx}.png"
            show(  # noqa: F821 - pre-existing vendored bug
                self.corr,
                fname=fname,
        ...
        self.update_cur_metric(
            recalc_metric=True, recalc_edges=True
        )  # so we log the correct state...
    ...
    def remove_redundant_node(self, node, safe=True, allow_fails=True):
        ...
        while bfs_idx < len(bfs):
            ...
            for child_node in children:
                if (
                    self.corr.edges[child_node.name][child_node.index][cur_node.name][
                        cur_node.index
                    ].edge_type.value
    ...
    def current_node_connected(self):
        for child_name, rest1 in self.corr.edges.items():
            for child_index, rest2 in rest1.items():
                if (
                    self.current_node.name in rest2
                    and self.current_node.index in rest2[self.current_node.name]
                ):
        ...
        for parent_name in parent_names:
            ...
            try:
                rest1 = self.corr.edges[self.current_node.name][self.current_node.index][
                    parent_name
                ]
            except KeyError:
                continue
        ...
        assert abs(self.cur_metric - old_metric) < 3e-3, (
            "Removing all incoming edges should not change the metric",
            self.cur_metric,
            old_metric,
            self.current_node,
        )
    ...
    def increment_current_node(self) -> None:
        while True:
            ...
            if (
                ...
                or self.current_node.name
                in ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed"]
            ):
                break
    ...
    def save_subgraph(self, fpath: Optional[str] = None, return_it=False) -> None:
        ...
        for tupl, edge in self.corr.all_edges().items():
            ...
            receiver_index, sender_index = (
                receiver_torch_index.hashable_tuple,
                sender_torch_index.hashable_tuple,
            )
    ...
    def load_subgraph(self, subgraph: Subgraph):
        ...
        for (
            ...
            sender_name,
            sender_index,
        ), is_present in subgraph.items():
            receiver_torch_index, sender_torch_index = TorchIndex(receiver_index), TorchIndex(
                sender_index
            )
            edge = self.corr.edges[receiver_name][receiver_torch_index][sender_name][
                sender_torch_index
            ]
        ...
        for i, line in enumerate(lines):
            ...
            if removing_connection or keeping_connection:
                ...
                for back_index in range(6):
                    previous_line = lines[
                        i - back_index
                    ]  # magic number because of the formatting of the log lines
                ...
                parent_torch_index, current_torch_index = TorchIndex(parent_list), TorchIndex(
                    current_list
                )
        ...
        for receiver_name in self.corr.edges:
            for receiver_index in self.corr.edges[receiver_name]:
                for sender_name in self.corr.edges[receiver_name][receiver_index]:
                    for receiever_index in self.corr.edges[receiver_name][receiver_index][
                        sender_name
                    ]:
                        ...
                        edge = self.corr.edges[receiver_name][receiver_index][sender_name][
                            receiever_index
                        ]
                        edge.present = False
