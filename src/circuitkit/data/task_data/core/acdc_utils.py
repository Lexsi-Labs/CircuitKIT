...
def negative_log_probs(
    ...
    nll_all = (
        F.nll_loss(logprobs.view(-1, logprobs.size(-1)), labels.view(-1), reduction="none").view(
            logprobs.size()[:-1]
        )
        - baseline
    )
...
def make_nd_dict(end_type, n=3) -> Any:
    ...
    # Build n-level nested defaultdict recursively.
    def _nest(depth):
        if depth == 1:
            return defaultdict(end_type)
        return defaultdict(lambda d=depth: _nest(d - 1))

    outer = OrderedDefaultdict(lambda: _nest(n - 1))
    return outer
...
def extract_info(string):
    ...
    if parent_list_str:
        ...
        parent_list = [
            ast.literal_eval(item if item != "COL" else "None") for item in parent_list_items
        ]
    ...
    if current_list_str:
        ...
        current_list = [
            ast.literal_eval(item if item != "COL" else "None") for item in current_list_items
        ]
...
def get_edge_stats(ground_truth, recovered):
    ...
    counts["all"] = len(
        [e for e in ground_truth_all_edges.values() if e.edge_type != EdgeType.PLACEHOLDER]
    )
...
def false_positive_rate(ground_truth, recovered, verbose=False):
    return get_stat(  # noqa: F821 - pre-existing vendored bug
        ground_truth, recovered, mode="false positive", verbose=verbose
    )


def false_negative_rate(ground_truth, recovered, verbose=False):
    return get_stat(  # noqa: F821 - pre-existing vendored bug
        ground_truth, recovered, mode="false negative", verbose=verbose
    )


def true_positive_stat(ground_truth, recovered, verbose=False):
    return get_stat(  # noqa: F821 - pre-existing vendored bug
        ground_truth, recovered, mode="true positive", verbose=verbose
    )
...
def reset_network(task: str, device, model: torch.nn.Module) -> None:
    ...
    reset_state_dict = torch.load(random_model_file, map_location=device, weights_only=True)
...
def get_time_diff(run_name):
    ...
    logger.info(arr[-1].item() - arr[0].item())
