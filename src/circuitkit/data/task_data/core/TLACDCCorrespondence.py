...
from .acdc_utils import OrderedDefaultdict, make_nd_dict
from .TLACDCEdge import Edge, EdgeType, TorchIndex  # these introduce several important classes !!!
...
class TLACDCCorrespondence:
    ...
    edges: MutableMapping[
        str, MutableMapping[TorchIndex, MutableMapping[str, MutableMapping[TorchIndex, Edge]]]
    ]

    def __init__(self):
        self.graph = OrderedDefaultdict(OrderedDict)
        self.edges = make_nd_dict(end_type=None, n=4)

    def first_node(self):
        return self.graph[list(self.graph.keys())[0]][
            list(self.graph[list(self.graph.keys())[0]].keys())[0]
        ]
    ...
    def remove_edge(
        ...
        try:
            ...
            raise ValueError(
                f"Edge does not exist: {(child_name, child_index, parent_name, parent_index)}"
            ) from e
