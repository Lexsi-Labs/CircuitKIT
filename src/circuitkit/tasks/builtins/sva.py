"""
SVA (Subject-Verb Agreement) Task Specification

Thin wrapper for subject-verb agreement task.
...
from ...data.task_data.tasks.sva.utils import generate_sva_data
...
logger = get_logger("task.sva")


class SVATaskSpec:
    """SVA task using simplified wrapper approach."""
    ...
    def validate_discovery_config(self, discovery_cfg: Dict[str, Any]) -> None:
        """Validate SVA-specific discovery configuration."""
        algorithm = discovery_cfg.get("algorithm", "").lower()
        if algorithm not in (EAP_FAMILY | IB_FAMILY | CDT_FAMILY):
            raise ValueError(
                unsupported_algorithm_message("SVA", algorithm, EAP_FAMILY | IB_FAMILY | CDT_FAMILY)
            )

        level = discovery_cfg.get("level")
        if level not in ["node", "neuron"]:
            raise ValueError(
                f"SVA discovery config has invalid 'level': {level!r}. "
        ...
        if algorithm == "ibcircuit":
            ...
            if scope not in ["heads", "mlp", "both"]:
                raise ValueError(
                    f"SVA ibcircuit has invalid 'scope': {scope!r}. "
        ...
        if not isinstance(batch_size, int) or batch_size <= 0:
            raise ValueError(
                f"SVA has invalid 'batch_size': {batch_size!r}. "
    ...
    def build_dataloader(
        ...
        """Build DataLoader for SVA task."""
        if model is None:
            raise ValueError("SVA task requires model for tokenizer.")

        # SVA prompts are generated by an external data module and the EAP
        # dataloader tokenizes the cached CSV internally -- there is no single
    ...
    def _get_or_generate_csv(self, discovery_cfg: Dict[str, Any], model) -> Path:
        """Return path to cached SVA CSV, generating it if absent."""
        ...
        cache_dir = Path(discovery_cfg.get("cache_dir", "./cache/sva"))
        cache_dir.mkdir(parents=True, exist_ok=True)

        model_name = getattr(model.cfg, "model_name", "unknown").replace("/", "_")
        data_path = cache_dir / f"sva_{model_name}_{n_samples}_seed{seed}.csv"
    ...
    def _build_eap_dataloader(
        ...
        logger.debug(
            f"[DEBUG PADDING] sva EAP dataloader  pair_padding_side='{side}'  batch_size={batch_size}"
        )
    ...
    def _build_ibcircuit_dataloader(self, discovery_cfg: Dict[str, Any], device: str, model):
        ...
        # Tokenize each context individually
        token_lists = [
        ...
        logger.debug(
            f"[DEBUG PADDING] sva IBCircuit  within-batch=right-padded  max_len={max_len}  answer_pos range=[{answer_positions.min().item()}, {answer_positions.max().item()}]"
        )
    ...
    def build_finetuning_dataset(
        ...
        """Load SVA finetuning data from discovery cache."""
        cfg = discovery_cfg or {}
        model_name_safe = model_name.replace("/", "_")
        cache_dir = Path(cfg.get("cache_dir", "./cache/sva"))
    ...
    def artifact_metadata(self, discovery_cfg: Dict[str, Any]) -> Dict[str, Any]:
        """Generate metadata for SVA artifacts."""
        return {
            "task": "sva",
