"""Validation: RelP / Relevance Patching (Mohebbi et al. 2025) on GPT-2 IOI.

Same EAP forward + backward cost. Gradient is re-routed by three forward
detach hooks (LN-rule on layer-norm scales, AH-rule on attention pattern,
Identity-rule on MLP activations) so backprop yields LRP-ε-style relevance
coefficients in place of raw gradients. Half-rule for gated MLPs is not
implemented; only applies to GeGLU/SwiGLU models — GPT-2 is non-gated.
