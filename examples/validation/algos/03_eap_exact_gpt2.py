"""Validation: top-level algorithm key 'eap-exact' on GPT-2 IOI.

Promoted in this session from `discovery.method='exact'`. Computes the
exact patching effect per node by individually patching each candidate
component (no gradient approximation). Cost is O(n_nodes * n_examples)
forward passes, so we use a deliberately small num_examples here.
...
def main() -> int:
    ...
    try:
        # Smaller num_examples because exact patching does one forward
        # per (candidate node, example) — wall time grows linearly.
