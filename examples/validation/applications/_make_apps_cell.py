"""Template-based generator for per-task pruning application cells.

Mirrors the pruning cell pattern from 01/03 — discover, prune low-30%
nodes via weight zeroing, measure task-specific metric vs random baseline.
"""
from __future__ import annotations
from pathlib import Path

PRUNE_TEMPLATE = '''"""Application: structural pruning on GPT-2 / {task} for every algorithm.
...
CELLS = [
    ("04_pruning_gpt2_gender_bias",     "gender_bias",
     "Pruning preserves gender-bias circuit (built-in TaskSpec).", False),
    ("05_pruning_gpt2_sva",             "sva",
     "Pruning preserves subject-verb agreement circuit.", False),
    ("06_pruning_gpt2_capital_country", "capital_country",
     "Pruning preserves capital-country factual recall.", False),
    ("07_pruning_gpt2_boolq",           "boolq",
     "Pruning preserves BoolQ yes/no QA on real HF data.", False),
    ("08_pruning_gpt2_arc_easy",        "arc_easy",
     "Pruning preserves ARC-Easy MCQ via NormalizedTaskSpec.", True),
    ("09_pruning_gpt2_hellaswag",       "hellaswag",
     "Pruning preserves HellaSwag commonsense MCQ.", True),
    ("10_pruning_gpt2_crows_pairs",     "crows_pairs",
     "Pruning preserves CrowS-Pairs bias detection.", True),
]


def main() -> None:
    out = Path(__file__).resolve().parent
    for script, task, desc, needs_custom in CELLS:
        path = out / f"{script}.py"
        body = PRUNE_TEMPLATE.format(
            script_name=script, task=task, description=desc, needs_custom=needs_custom,
        )
