"""Benchmark: all 9 algorithms on Llama-3.2-1B + IOI.

Llama uses RMSNorm + GeGLU MLPs. The RelP cell here will be missing the
Half-rule (gated-MLP rule), so its score is "RelP minus Half-rule" — a
known gap, see ``backends/eap/attribute_node.py::_build_relp_hooks``.
"""
...
def main() -> int:
    ...
    for algo in ALL_ALGOS:
        # Llama-1B is roughly 7x GPT-2 in params; halve num_examples again
        # for the slow algos to keep cell wall in the 1-3 minute range.
        if algo in ("eap-exact",):
            num_examples = 8
