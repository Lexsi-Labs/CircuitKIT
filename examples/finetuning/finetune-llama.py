"""
finetune_llama.py — Selective finetuning pipeline for LLaMA family models.

Phases
------
1. Load circuit discovery scores (score_loader).
2. Read model config via AutoConfig → derive n_layers, n_q_heads, n_kv_heads,
   head_dim → call selector to produce circuit / random / baseline components.
3. Load AutoTokenizer + LlamaForCausalLM. Evaluate base accuracy.
4. Build finetuning DataLoader (LanguageModelingDataset format).
5. For each condition in order: circuit → random → baseline
     a. Deep-copy base model onto GPU.
     b. Run selective finetuning (run_finetuning handles hooks + cleanup).
     c. Evaluate finetuned model.
     d. Delete copy and flush GPU cache before next condition.
6. Print results table + per-epoch curves. Optionally save circuit model.

Usage
-----
    python finetune_llama.py \\
        --scores-path outputs/eap-ig_ioi_llama3_scores.pt \\
        ...
        --n-epochs    3   \\
        --lr          2e-5 \\
        --output-dir  outputs/finetune_llama_ioi
"""
...
import circuitkit.applications.selective_finetuning as _sft  # score_loader
# and selector live in the package; the script was relocated to
# examples/finetuning/, so the old dirname(dirname(__file__)) no longer
# pointed at selective_finetuning/.
parent_dir = os.path.dirname(os.path.abspath(_sft.__file__))
...
# ---------------------------------------------------------------------------
# Task registry — maps short aliases to fully-qualified class paths.
# Extend this dict to register new circuitkit task types without changing
# any other code.
# ---------------------------------------------------------------------------
...
def _read_model_config(model_name: str) -> Dict[str, int]:
    """
    ...
    # GQA: num_key_value_heads <= num_attention_heads. Falls back to MHA if absent.
...
def _build_eval_dataloader(
    ...
    print("\n[DEBUG] Data Check - Raw Strings")
    for i in range(2):
        print(f"  clean_text [{i}]: {repr(clean_texts[i])}")
        print(f"  query_text [{i}]: {repr(query_strings[i])}")
    ...
    def eval_fn(model: torch.nn.Module, eval_dl) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in eval_dl:
                ...
                if total == 0:  # Only print for the very first batch
                    print("\n[DEBUG] Eval Batch 1 - Indexing & Prediction Check")
                    for i in range(min(2, input_ids.size(0))):
                        qlen = query_lengths[i].item()

                        # What token is the model looking at to make its prediction?
                        last_ctx_id = input_ids[i, qlen - 1].item()
                        # What is it supposed to predict?
                        target_id = ans[i].item()
                        # What did it actually predict?
                        pred_id = preds[i].item()

                        print(f"  Ex {i} | query_length index: {qlen}")
                        print(f"  Ex {i} | Context end ID  : {last_ctx_id}")
                        print(f"  Ex {i} | Target ID       : {target_id}")
                        print(f"  Ex {i} | Prediction ID   : {pred_id}")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Selective finetuning pipeline for LLaMA family models.",
    ...
    req.add_argument(
        "--model-name",
        required=True,
        help="HuggingFace model identifier " "(e.g. meta-llama/Meta-Llama-3-8B).",
    )
    req.add_argument(
        "--task",
        required=True,
    ...
    trn.add_argument(
        "--seed", type=int, default=0, help="Global RNG seed (controls DataLoader shuffle etc.)."
    )
    ...
    mdl.add_argument(
        ...
        help="Model dtype for inference and training.",
    )
    mdl.add_argument(
    ...
    out.add_argument(
        "--output-dir",
        default="outputs/finetune_llama",
...
def main(args: argparse.Namespace) -> None:
    ...
    # Fall back to CPU gracefully if CUDA is unavailable and user kept default.
    device = torch.device(
    ...
    print("=" * 60)
    arch = _read_model_config(args.model_name)
    print(
    ...
    # Bundle arguments shared across all three selector calls to avoid repetition.
    shared_sel = dict(
    ...
    print(f"[pipeline] Loading LlamaForCausalLM ({args.dtype}) ...")
    t0 = time.time()
    ...
    print(f"[pipeline] Model loaded in {time.time() - t0:.1f}s.")
    ...
    finetune_dl = build_finetune_dataloader(
        task_spec=task_spec,
        tokenizer=tokenizer,
        model_name=stripped_model_name,  # for cache path reconstruction
    ...
    for cond_name, selection in conditions:
        ...
        try:
            ...
            # Optionally persist the circuit-finetuned model weights.
        ...
        json.dump(
            {
                "args": vars(args),
                "arch": arch,
                # score_meta may contain Tensors (neuron-level MLP values);
                # filter them so the dict is JSON-serialisable.
                "score_meta": {
...
if __name__ == "__main__":
    ...
    # Fast sanity checks before any expensive work starts.
