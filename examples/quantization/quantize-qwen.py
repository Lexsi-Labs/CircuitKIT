"""
quantize_qwen.py — Full circuit-guided quantization pipeline for Qwen3 models.

Differences from quantize_llama.py
------------------------------------
* Uses Qwen3ForCausalLM instead of LlamaForCausalLM.
* Default task is "mmlu" to match Qwen's typical training distribution.
* Qwen3 attention layers contain per-head q_norm and k_norm (Qwen3RMSNorm).
  These are not nn.Linear layers so optimum-quanto skips them automatically —
  no special handling required.

Uses node-level discovery (is_neuron=False, mlp_hook="mlp_out") — one scalar
score per attention head and one per MLP block.
Pipeline and comparison table are otherwise identical to quantize_llama.py.
...
python quantize_qwen.py \\
    ...
    --save-scores scores/qwen3_mmlu.pt

# Re-quantize from saved scores:
python quantize_qwen.py \\
    --base-model Qwen/Qwen3-0.6B \\
    --scores-path scores/qwen3_mmlu.pt \\
    --eval-data-path scores/qwen3_mmlu_eval.pt \\
    --low-weights qint4 --high-weights qint8 --high-fraction 0.3 \\
    --compare-uniform --eval-ppl
"""
...
for _p in [_QUANTO_ROOT, _QUANT_APP]:
...
from circuitkit.applications.quantization.quant_utils import (  # noqa: E402 - import after intentional pre-import setup
...
from circuitkit.applications.quantization.score_extractor import (  # noqa: E402 - import after intentional pre-import setup
    collect_eval_data,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Circuit-guided mixed-precision quantization for Qwen3",
